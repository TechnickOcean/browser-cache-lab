...
// [中文注释]
// http_cache.cc 是 Chromium 网络栈中 HTTP 缓存的核心实现文件。
// 它主要负责管理 HTTP 响应的存储和检索，充当 HttpTransactionFactory，
// 创建 HttpCache::Transaction 来处理具体的 HTTP 请求。
// HttpCache 类协调了内存中的 ActiveEntry（正在使用的缓存条目）
// 和底层的磁盘缓存后端 (disk_cache::Backend)。
// 它处理了复杂的并发场景，例如多个请求同时访问同一个 URL (writer/reader模型)，
// 以及缓存条目的创建、打开、失效（Doom）等生命周期管理。
...
// [中文注释]
// HttpCache 使用双键 (double keying) 策略来隔离缓存，防止跨站信息泄露。
// kDoubleKeyPrefix 是双键缓存键的前缀，用于区分普通单键缓存。
const char HttpCache::kDoubleKeyPrefix[] = "_dk_";
// kDoubleKeySeparator 是双键缓存键的分隔符。
const char HttpCache::kDoubleKeySeparator[] = " ";
// kSubframeDocumentResourcePrefix 用于标记子框架文档资源的缓存键前缀。
const char HttpCache::kSubframeDocumentResourcePrefix[] = "s_";
// kCrossSiteMainFrameNavigationPrefix 用于标记跨站主框架导航的缓存键前缀。
const char HttpCache::kCrossSiteMainFrameNavigationPrefix[] = "cn_";

// [中文注释]
// DefaultBackend 是 HttpCache::BackendFactory 的默认实现。
// 它负责根据配置创建磁盘缓存 (disk_cache) 后端实例。
...
// [中文注释]
// 实际上创建后端实例的方法。
// 它调用 disk_cache::CreateCacheBackend 来完成工作。
...
// [中文注释]
// ActiveEntry 代表一个正在内存中处于活动状态的缓存条目。
// 当有一个或多个 Transaction 正在读写某个缓存项时，该缓存项就会被包装成 ActiveEntry。
// 它维护了读写该条目的 Transaction 列表，以及相关的状态（如是否已失效 doomed）。
HttpCache::ActiveEntry::ActiveEntry(base::WeakPtr<HttpCache> cache,
  ...
  CHECK(disk_entry_);
  // [中文注释] 将自己注册到 cache_->active_entries_ 映射表中。
...
// [中文注释]
// 当被标记为失效 (doomed) 的条目彻底销毁时调用。
// 从 cache_->doomed_entries_ 集合中移除自己。
...
// [中文注释]
// 将自己从 cache_->active_entries_ 活跃列表中移除。
// 通常在最后一个引用释放时调用。
...
// [中文注释]
// 释放 Writers 对象。Writers 负责协调并发写入。
...
// [中文注释]
// 将一个 Transaction 添加到 Writers 列表中。
// 如果这是第一个写入者，会创建 Writers 对象。
// 这里处理了并发写入的模式 (parallel_writing_pattern)。
...
// [中文注释]
// 标记该条目已失效 (Doom)。这意味着后续请求不能再使用它，
// 即使当前还有 Transaction 在使用它。
...
// [中文注释]
// 处理等待进入 Entry 的队列。
// 如果没有 Transaction 正在处理头部，则取出队列中的第一个 Transaction 开始处理。
void HttpCache::ActiveEntry::ProcessAddToEntryQueue() {
  ...
  // [中文注释] 同一时刻只能有一个 Transaction 处于 headers 阶段。
  if (headers_transaction_) {
    return;
  }
...
// [中文注释]
// PendingOp 结构体用于跟踪那些试图创建或打开缓存条目，或者创建后端本身的操作。
// 当多个请求同时试图操作同一个 key 时，PendingOp 会将后续请求排队。
struct HttpCache::PendingOp {
  PendingOp() = default;
  ~PendingOp() = default;

  // [中文注释] 指向相关的 disk_cache::Entry。
  raw_ptr<disk_cache::Entry> entry = nullptr;
  bool entry_opened = false;  // rather than created.

  // [中文注释] 指向刚创建的 backend（如果是创建后端的操作）。
  std::unique_ptr<disk_cache::Backend> backend;
  // [中文注释] 当前正在执行的 WorkItem（主操作）。
  ...
  // [中文注释] 等待同一个 key 操作完成的其他 WorkItem 队列。
...
// [中文注释]
// WorkItem 封装了对后端的一个请求（操作），包含了完成该请求所需的所有信息。
// 操作类型包括：打开、创建、Doom 缓存项，或创建后端。
...
HttpCache::HttpCache(
  ...
  // [中文注释] 标记缓存已初始化。
  g_init_cache = true;
  // [中文注释] 如果启用了 NoVarySearch 特性，初始化相关缓存。
  ...
  // [中文注释] 如果启用了早期初始化磁盘缓存后端特性，则尝试尽早创建后端。
...
HttpCache::~HttpCache() {
  ...
  // [中文注释] 使所有 WeakPtr 失效，防止后续回调访问已销毁的对象。
  ...
  // [中文注释] 销毁磁盘缓存后端。必须在清理 pending_ops_ 之前进行。
  disk_cache_.reset();
...
// [中文注释]
// 获取缓存后端。如果已经存在，直接通过 callback 返回。
// 如果不存在，则触发创建流程。
...
// [中文注释]
// 关闭所有连接。这通常是在出现严重错误时调用。
// 委托给底层的 HttpNetworkSession。
...
// [中文注释]
// 当外部缓存（例如 Service Worker 或其他类型的缓存）发生命中时调用。
// 用于更新磁盘缓存中的 LRU（最近最少使用）信息，保持活跃条目不被驱逐。
...
// static
// [中文注释]
// 从缓存 key 中解析出原始的资源 URL。
// 缓存 key 格式通常包含 credential_key、upload_data_identifier、[isolation_key] 和 url。
...
// [中文注释]
// 生成用于在磁盘缓存中索引条目的 key。
// key 的构成取决于是否启用 SplitCache（缓存分区）。
// 如果启用了 SplitCache，key 会包含 NetworkIsolationKey，实现资源隔离。
std::string HttpCache::GenerateCacheKey(
  ...
  // [中文注释] 如果启用了按凭据分割缓存，key 的第一位表示是否保存 Cookie。
...
// [中文注释]
// 创建并设置一个 WorkItem，然后将其添加到 PendingOp 的等待队列中。
...
// [中文注释]
// 为特定 Transaction 获取 Backend。
// 这主要用于在 Backend 初始化之前就发起请求的场景。
...
// [中文注释]
// 删除（失效）一个缓存条目。
// 如果该条目是活跃的，将其移动到 doomed_entries_ 列表。
// 如果不活跃，则异步调用磁盘缓存的 DoomEntry。
...
// [中文注释]
// 获取活跃条目。如果条目已被逻辑失效（通过 InvalidationFilter），则将其 Doom 并返回 nullptr。
...
// [中文注释]
// 将磁盘缓存条目激活为 ActiveEntry。
// 如果该条目已被逻辑失效，则立即 Doom 并返回 nullptr。
...
// [中文注释]
// 查找或创建 PendingOp。
// 如果对于给定的 key 已经有 PendingOp，则返回它。
// 否则，创建一个新的 PendingOp 并存储在 pending_ops_ 映射中。
...
// [中文注释]
// 将 Transaction 添加到 ActiveEntry 的等待队列中。
// 即使是第一个 transaction，也总是先加入队列，以保持 FIFO 顺序。
...
// [中文注释]
// 当 Transaction 完成响应头的写入或读取时调用。
// 这个方法负责将 Transaction 从 headers 阶段转换到 body 读/写阶段，
// 或者在某些情况下将其加入 done_headers_queue。
...
// [中文注释]
// 当 Transaction 完成对 Entry 的使用时调用（无论是成功还是中途取消）。
// 负责清理资源，并可能触发后续等待的 Transaction。
...
// [中文注释]
// 当 Writers 完成对 Entry 的写入时调用。
// success: 写入是否成功。
// should_keep_entry: 是否保留该条目（例如，对于截断的条目）。
// make_readers: 哪些 transaction 应该转为 reader。
void HttpCache::WritersDoneWritingToEntry(scoped_refptr<ActiveEntry> entry,
  ...
  if (success) {
    ...
    // [中文注释] 将之前作为 writer 的事务转换为 reader。
...
// [中文注释]
// 当验证（304 Not Modified）失败，即服务器返回非 304 响应时调用。
// 这意味着缓存内容已过时，需要 Doom 旧条目并重新发起请求。
void HttpCache::DoomEntryValidationNoMatch(scoped_refptr<ActiveEntry> entry) {
  ...
  // [中文注释] 重启所有在 add_to_entry_queue 中的事务。
...
// [中文注释]
// 处理 Entry 失败的情况（如写入失败）。
// Doom 该条目，并通知所有相关事务重启（ERR_CACHE_RACE）。
...
// [中文注释]
// 处理排队的事务。
// 这是一个调度方法，确保在正确的时机（避免重入）处理 done_headers_queue 和 add_to_entry_queue。
...
// [中文注释]
// 检查当前事务是否可以加入现有的 Writers 进行并行写入。
...
// [中文注释]
// 处理完成 headers 处理的事务队列。
// 决定将事务转换为 reader 还是 writer，或者继续等待。
...
// [中文注释]
// I/O 操作完成时的通用处理函数。
// 根据操作类型 (PendingOp) 和结果 (result)，更新 Entry 状态并通知 Transaction。
void HttpCache::OnIOComplete(int result, PendingOp* pending_op) {
  ...
  if (result == OK) {
    if (op == WI_DOOM_ENTRY) {
      ...
      // [中文注释] Doom 操作成功后，所有排队的请求都需要重启，因为旧条目已失效。
      try_restart_requests = true;
    } else if (item->IsValid()) {
      DCHECK(pending_op->entry);
      key = pending_op->entry->GetKey();
      // [中文注释] 尝试激活 Entry。如果 Entry 被逻辑失效，则返回 nullptr。
  ...
  while (!pending_items.empty()) {
    ...
    if (item->operation() == WI_CREATE_ENTRY) {
      if (result == OK) {
        ...
        // [中文注释] 如果已有 Entry 且操作成功，但后续请求是 Create，则失败（因为不能覆盖）。
...
// [中文注释]
// 后端创建完成后的回调。
// 设置 disk_cache_ 指针，并处理所有等待后端创建的 WorkItems。
...
// [中文注释]
// 检查 Entry 是否被任何当前的 InvalidationFilter 标记为失效。
// 这用于支持 "清除浏览数据" 功能，根据时间范围和来源删除数据。
